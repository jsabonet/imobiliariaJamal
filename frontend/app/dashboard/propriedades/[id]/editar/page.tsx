...
import { 
  ...
  setPrimaryImage as setImageAsPrimary 
} from '@/lib/api';
import { FiSave, FiX, FiChevronDown, FiChevronUp, FiHome, FiMapPin, FiDollarSign, FiGrid, FiStar, FiSettings, FiImage, FiLock, FiTrash2, FiCheckCircle } from 'react-icons/fi';
...
export default function EditarPropriedadePage() {
  ...
  const handleSetPrimaryImage = async (imageId: number) => {
    try {
      ...
      await setImageAsPrimary(propId, imageId);
  ...
  // Carregar dados da propriedade existente
  useEffect(() => {
    const loadPropertyData = async () => {
      try {
        ...
        // Preencher formData com todos os campos
        setFormData({
          title: data.title || '',
      ...
      <DashboardLayout>
        <div className="p-4 md:p-6 lg:p-8 flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <p className="text-gray-600 text-lg">Carregando dados da propriedade...</p>
          </div>
        </div>
      </DashboardLayout>
    );
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="p-4 md:p-6 border-t border-gray-100 animate-fadeIn space-y-4">
                ...
                {existingImages.length > 0 && (
                  <div className="space-y-3">
                    <h4 className="text-sm font-bold text-gray-700">Imagens Atuais ({existingImages.length})</h4>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {existingImages.map((img) => {
                        const API_BASE = (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api').replace(/\/?api\/?$/, '');
                        const imageUrl = img.image.startsWith('http') ? img.image : `${API_BASE}${img.image}`;
                        return (
                          <div 
                            key={img.id} 
                          ...
                          >
                            ...
                            {img.is_primary && (
                              <div className="absolute top-2 left-2 bg-accent text-white text-xs px-2 py-1 rounded flex items-center gap-1">
                            ...
                            <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                              {!img.is_primary && (
                                <button
                                  ...
                                  className="bg-accent text-white px-3 py-1 rounded text-xs hover:bg-accent-600 flex items-center gap-1"
            ...
            {expandedSections.documents && (
              <div className="p-4 md:p-6 border-t border-gray-100 animate-fadeIn space-y-4">
                ...
                {existingDocuments.length > 0 && (
                  <div className="space-y-3">
                    <h4 className="text-sm font-bold text-gray-700">Documentos Atuais ({existingDocuments.length})</h4>
                    <div className="space-y-2">
                      {existingDocuments.map((doc) => {
                        const API_BASE = (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api').replace(/\/?api\/?$/, '');
                        const docUrl = doc.document.startsWith('http') ? doc.document : `${API_BASE}${doc.document}`;
                        return (
                          <div 
                            key={doc.id} 
                            ...
                            <div className="flex items-center gap-3 flex-1">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-gray-500">
                                  {new Date(doc.uploaded_at).toLocaleDateString('pt-MZ')}
                                </p>
                              </div>
                            </div>
                ...
                <label className="block text-sm font-medium text-secondary-700 mb-2">
                  Upload de Novos Documentos (PDF, JPG, PNG) - Apenas para Arquivo Interno
                </label>
                <input
                  type="file"
                  multiple
          ...
          <div className="sticky bottom-0 bg-white border-t border-gray-200 p-4 -mx-4 md:-mx-6 lg:-mx-8 mt-8 shadow-lg md:shadow-none md:static md:border-0 md:p-0 md:m-0">
            <div className="flex flex-col md:flex-row gap-3 md:gap-4">
              ...
              >
                <FiSave size={20} />
                <span className="font-semibold">{loading ? 'Salvando...' : 'Salvar Alterações'}</span>
              </Button>
              <Button
                type="button"
                variant="outline"
